...
common::result::Result<void> WorkerDeployService::validate(const WorkerDeployRequest& req) {
    ...
    if (req.grpc_port < 0 || req.grpc_port > 65535) {
        return common::result::Result<void>::failure("grpc_port 必须在 0-65535 之间（0 表示自动分配）");
    }
...
std::string WorkerDeployService::generateWorkerConfig(const WorkerDeployRequest& req) const {
    // The worker's advertised address (what the scheduler dials back to) is
    // host:grpc_port. 端口自动分配时（grpc_port == 0）交给 worker 自己算：
    // 只有 worker 知道内核最终分配了哪个端口，它会把
    // 「本机可路由 IPv4 + 实际端口」注册上来。
    const bool auto_port = req.grpc_port <= 0;
    std::string advertise_address =
        auto_port ? "auto" : req.host + ":" + std::to_string(req.grpc_port);

    std::ostringstream ss;
    ss << "# Auto-generated by TaskFlow scheduler worker deployment\n";
    ss << "server:\n";
    if (!grpc_auth_token_.empty()) {
        // Fix #357: 注入 scheduler 自身的内部认证 token（服务端校验 +
        // 作为访问 scheduler 的客户端凭证），与 scheduler.yaml 保持一致。
        // 必须在 server: 块内——顶层混合缩进会破坏 YAML 结构，
        // 导致 worker 解析不到 scheduler.address 而回退 localhost。
        ss << "  grpc_auth_token: " << yamlDoubleQuote(grpc_auth_token_) << "\n";
    }
    ss << "  grpc_port: " << (auto_port ? "auto" : std::to_string(req.grpc_port)) << "\n";
...
common::result::Result<WorkerDeployResult> WorkerDeployService::deploy(const WorkerDeployRequest& req) {
    ...
    // 端口自动分配时真实地址要等 worker 注册后才知道，这里给出可读的占位
    result.address = req.grpc_port > 0
                         ? req.host + ":" + std::to_string(req.grpc_port)
                         : req.host + ":auto";
