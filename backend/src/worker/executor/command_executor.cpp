...
TaskResult CommandExecutor::execute(const std::string& task_instance_id,
    ...
    if (pid == 0) {
        ...
        // process), not grandchildren (e.g. `sleep` forked by
        // `/bin/sh -c "sleep 300"`). So we also fork a watchdog that monitors
        // this process: when it is reparented to init (parent died), it kills
        // the entire process group with SIGKILL, cleaning up grandchildren.
#ifdef __linux__
        ...
        if (watchdog_pid == 0) {
            // Watchdog: poll parent liveness. When the child (our parent) is
            // reaped by init, getppid() becomes 1 — kill the whole group.
            while (getppid() != 1) {
                struct timespec ts;
                ts.tv_sec = 0;
